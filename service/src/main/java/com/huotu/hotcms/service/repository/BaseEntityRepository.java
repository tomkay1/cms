...
public interface BaseEntityRepository extends JpaRepository<BaseEntity,Long>,JpaSpecificationExecutor {
    ...
    @Query(value = "select a.title,a.description,c.name,a.id,c.modelId,a.createTime from cms_link a,cms_category c where a.categoryId=c.id and a.deleted=false and c.siteId = ?1 and a.title like %?2%\n" +
            "UNION\n" +
            ...
            "UNION\n" +
            "select g.title,g.description,c.name,g.id,c.modelId,g.createTime from cms_gallery g,cms_category c where g.categoryId=c.id and g.deleted=false and c.siteId = ?1 and g.title like %?2%",nativeQuery = true)
    List<Object[]> findAllContentsBySiteIdAndName(Long siteId,String name);

    @Query(value = "select a.title,a.description,c.name,a.id,c.modelId,a.createTime from cms_link a,cms_category c where a.categoryId=c.id and a.deleted=false and c.siteId = ?1 and c.id=?2 and a.title like %?3%\n" +
            "UNION\n" +
            ...
            "UNION\n" +
            "select g.title,g.description,c.name,g.id,c.modelId,g.createTime from cms_gallery g,cms_category c where g.categoryId=c.id and g.deleted=false and c.siteId = ?1 and c.id=?2 and g.title like %?3%",nativeQuery = true)
    List<Object[]> findAllContentsBySiteIdAndCategoryIdAndName(Long siteId,Long categoryId,String name);
}
